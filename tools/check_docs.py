#!/usr/bin/env python3
"""Small offline checks for this documentation repository, not a Markdown parser.

Checks simple inline file links outside fenced/inline code. Fragments, reference
links, and external URLs are deliberately not verified. Uses the standard library.
"""
...
REQUIRED = (
    "README.md", "AGENTS.md", "rules/WORKFLOW.md", "rules/MODEL_SELECTION.md",
    "rules/MODEL_CATALOG.md", "rules/VERSION", "rules/CHANGELOG.md",
)
...
def check_links(path: Path, text: str, root: Path) -> list[str]:
    """Check local paths and keep rules/ links inside the portable package."""
    errors: list[str] = []
    portable = root / "rules"
    ...
    for match in LINK_RE.finditer(prose_only(text)):
        ...
        if url.scheme or url.netloc or url.path.startswith("/"):
            ...
            continue
        if not url.path:
            continue  # Same-file fragments are outside the checker's scope.
...
def check_tree(root: Path) -> list[str]:
    """Return all structural problems without network calls or file mutations."""
    root = root.resolve()
    if not root.is_dir():
    ...
    version = texts.get("rules/VERSION", "").strip()
    if not VERSION_RE.fullmatch(version):
        errors.append("rules/VERSION: expected MAJOR.MINOR.PATCH or MAJOR.MINOR.PATCH-rc.N")
    elif not re.search(r"^## " + re.escape(version) + r"(?:\s|$)",
                       texts.get("rules/CHANGELOG.md", ""), flags=re.MULTILINE):
        errors.append("rules/CHANGELOG.md: current version heading is missing")
    return errors
...
def main() -> int:
    ...
    print("Not checked: anchors, reference links, external URLs, semantic correctness")
    return 0
